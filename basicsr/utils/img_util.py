...
def img2tensor(imgs, colorspace='RGB', float32=True):
    """Numpy array to tensor, with optional color space conversion.

    Args:
        imgs (list[ndarray] | ndarray): Input images.
        bgr2rgb (bool): Whether to change bgr to rgb. Applied only if colorspace is 'RGB'.
        float32 (bool): Whether to change to float32.
        colorspace (str): Target color space ('RGB', 'HSV', or 'Lab').

    Returns:
        list[tensor] | tensor: Tensor images. If returned results only have one element, just return tensor.
    """
    
    def _totensor(img, colorspace, float32):
        if colorspace == 'RGB':
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        elif colorspace == 'HSV':
    ...
    if isinstance(imgs, list):
        return [_totensor(img, colorspace, float32) for img in imgs]
    else:
        return _totensor(imgs, colorspace, float32)


def tensor2img(tensor, colorspace='RGB', out_type=np.uint8, min_max=(0, 1)):
    """Convert torch Tensors into image numpy arrays.
...
def padding(img_lq, img_gt, img_seg, gt_size):
    h, w, _ = img_lq.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lq, img_gt, img_seg

    img_lq = cv2.copyMakeBorder(img_lq, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_seg = cv2.copyMakeBorder(img_seg, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
