...
#     if not (input_folder.endswith('.lmdb') and gt_folder.endswith('.lmdb') and seg_folder.endswith('.lmdb')):
#         raise ValueError(
#             f'{input_key} folder and {gt_key} and {seg_key} folder should both in lmdb '
...
def paired_paths_from_lmdb(folders, keys):
    ...
    with open(osp.join(gt_folder, 'meta_info.txt')) as fin:
        gt_lmdb_keys = [line.split('.')[0] for line in fin]
    with open(osp.join(seg_folder, 'meta_info.txt')) as fin:
        seg_lmdb_keys = [line.split('.')[0] for line in fin]    
    
    if set(input_lmdb_keys) != set(gt_lmdb_keys) or set(input_lmdb_keys) != set(seg_lmdb_keys):
        raise ValueError(
            f'Keys in {input_key}_folder and {gt_key}_folder are different.')
    else:
        paths = []
...
def paired_paths_from_meta_info_file(folders, keys, meta_info_file,
    ...
    for gt_name in gt_names:
        ...
        input_name = f'{filename_tmpl.format(basename)}{ext}'
        input_path = osp.join(input_folder, input_name)
        seg_name = f'{filename_tmpl.format(basename)}{ext}'
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(input_paths) == len(gt_paths) == len(seg_paths), \
        f'Different numbers of images in folders: {len(input_paths)}, {len(gt_paths)}, {len(seg_paths)}.'
    ...
    for idx in range(len(gt_paths)):
        ...
        seg_name = f'{filename_tmpl.format(basename)}{ext_seg}'
        seg_path = osp.join(seg_folder, seg_name)
